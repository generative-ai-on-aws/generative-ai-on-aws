...
# Dec 2023, Chris Fregly: Modified for NeuronTrainer 
import inspect
import random
import warnings
...
class NeuronDPOTrainer(NeuronTrainer):
    r"""
    Initialize NeuronDPOTrainer.
    ...
    def __init__(
        ...
        args: NeuronTrainingArguments = None,
        ...
        if model_init_kwargs is None:
            ...
            raise ValueError("You passed model_kwargs to the NeuronDPOTrainer. But your model is already instantiated.")

        if ref_model_init_kwargs is None:
            ref_model_init_kwargs = {}
        elif not isinstance(ref_model, str):
            raise ValueError(
                "You passed ref_model_kwargs to the NeuronDPOTrainer. But your ref_model is already instantiated."
            )

        if isinstance(model, str):
            warnings.warn(
                "You passed a model_id to the NeuronDPOTrainer. This will automatically create an "
        ...
        if isinstance(ref_model, str):
            warnings.warn(
                "You passed a ref model_id to the NeuronDPOTrainer. This will automatically create an "
                "`AutoModelForCausalLM`"
            )
        ...
        if data_collator is None:
            ...
            if max_length is None:
                warnings.warn(
                    "When using DPODataCollatorWithPadding, you should set `max_length` in the NeuronDPOTrainer's init"
            ...
            if max_prompt_length is None:
                warnings.warn(
                    "When using DPODataCollatorWithPadding, you should set `max_prompt_length` in the NeuronDPOTrainer's init"
            ...
            if max_target_length is None and self.is_encoder_decoder:
                warnings.warn(
                    "When using DPODataCollatorWithPadding with an encoder decoder architecture, you should set `max_target_length` in the NeuronDPOTrainer's init"
                    " it will be set to `128` by default, but you should do it yourself in the future.",
                    UserWarning,
                )
        ...
        if not hasattr(self, "accelerator"):
            raise AttributeError(
                "Your `NeuronTrainer` does not have an `accelerator` object. Consider upgrading `transformers` or `optimum-neuron`."
            )
    ...
    def get_eval_dataloader(self, eval_dataset: Optional[Dataset] = None) -> DataLoader:
        ...
        if eval_dataset is None and self.eval_dataset is None:
            raise ValueError("NeuronTrainer: evaluation requires an eval_dataset.")
    ...
    def evaluation_loop(
        ...
        Prediction/evaluation loop, shared by `NeuronTrainer.evaluate()` and `NeuronTrainer.predict()`.
